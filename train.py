...
from config import get_weights_file_paths, get_config
...
def train_model(config):
    ...
    if config["preload"]:
        model_filename = get_weights_file_paths(config, config["preload"])
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        model_filename = get_weights_file_paths(config, f"{epoch:02d}")
        torch.save(
            {
                "epoch": epoch,
